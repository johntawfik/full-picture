...
import { getMockTimelinePerspectives, getAvailableSources } from "@/lib/mock-data"
...
export function ResultsTimeline({ query }: ResultsTimelineProps) {
  const timelineData = getMockTimelinePerspectives(query)
  const [selectedDate, setSelectedDate] = useState(timelineData[0].date)
